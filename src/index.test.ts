...
import type { RouteConfigToTypedResponse, WebhookConfig } from './index'
...
describe('Webhook', () => {
  ...
  it('Should register webhook in OpenAPI document', () => {
    ...
    expect(doc.webhooks).toHaveProperty('/webhooks/payment')
    expect(doc.webhooks!['/webhooks/payment']).toHaveProperty('post')
    expect(doc.webhooks!['/webhooks/payment'].post).toMatchObject({
      requestBody: {
        required: true,
        content: {
          'application/json': {
  ...
  it('Should register webhook in OpenAPI 3.1 document', () => {
    const doc31 = app.getOpenAPI31Document({
      openapi: '3.1.0',
      info: {
        title: 'My API',
    ...
    expect(doc31.webhooks).toHaveProperty('/webhooks/payment')
    expect(doc31.webhooks!['/webhooks/payment']).toHaveProperty('post')
  })

  it('Should support hidden webhooks', () => {
    const hiddenWebhook = createWebhook({
      method: 'post',
    ...
    // If no webhooks are registered, the webhooks property should be undefined or an empty object
    expect(doc.webhooks == null || Object.keys(doc.webhooks).length === 0).toBe(true)
  })
  ...
  it('Should verify webhook type correctness', () => {
    ...
    expect(() => createWebhook(webhookConfig)).not.toThrow()
  })
})
